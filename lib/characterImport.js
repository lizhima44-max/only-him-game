...
export function fillDefaults(json) {
  return {
    ...
    lastMoment: json.lastMoment || '',  // 👈 加这一行
  }
}
...
// 获取用户所有自定义角色列表（仅元数据，不加载完整 character_data）
export async function listCustomCharacters(supabase, userId) {
  ...
  if (error) {
    console.error('[CHAR] 列表查询失败:', error)
    return []
  }
  
  // 返回基础的 ID 列表，完整数据按需加载
  return data.map(item => ({
    id: item.character_id,
    name: '加载中...',   // 临时占位，真实名字后面异步加载
    tagline: '自定义角色',
    tags: [],
    characterData: null,
...
export async function uploadCharacterImage(supabase, userId, file, oldImageUrl) {
  if (!file) return null
