...
export function getShopItems(ownedWardrobe, ownedBedside, intimacy) {
  ...
  // 注意：这里 ownedWardrobe 是合并后的（女主+男主的衣服）
  // 但我们需要区分男女主的衣服分别显示
  // 更好的做法是分开传入，但为了兼容，先这样写
  // 从 ALL_OUTFITS 里取所有未拥有的衣服（包括男装和女装）
  const allClothes = ALL_OUTFITS
    .filter(o => !ownedWardrobe.includes(o.id) && o.price > 0 && intimacy >= o.unlockAt)
    .map(o => ({ ...o, shopType: 'wardrobe' }))

  const hisClothes = allClothes.filter(o => o.owner !== 'her')  // 男主的衣服
  const herClothes = allClothes.filter(o => o.owner === 'her')  // 女主的衣服
...
export const SUPERMARKET_ITEMS = [
  ...
  // 在 SUPERMARKET_ITEMS 的 life 分类里（末尾）添加：
  { id: 'pads',     name: '卫生巾',   emoji: '🩹', price: 15, category: 'life',  bedsideOnly: true },
  { id: 'condom',   name: '安全措施', emoji: '💊', price: 12, category: 'life',  bedsideOnly: true },
]
