...
export function processNewDay(dayState, realWeather) {
  ...
  if (isPeriod) events.push('🩸 生理期中，他会更体贴')
    // 经期最后一天提醒
if (isLastPeriodDay(dayState.periodDays || [], rd)) {
  events.push('💬 今天应该是生理期最后一天，可以问问她感觉怎么样～')
}
  if (sickWho === 'player') events.push('🤒 她今天不太舒服')
...
// 获取一段标记日期的连续范围（自动补全不连续的情况）
export function getPeriodRange(periodDays) {
  if (!periodDays || periodDays.length === 0) return null
  
  // 把所有标记转成Date对象并排序
  const dates = periodDays.map(key => new Date(key)).sort((a, b) => a - b)
  
  // 如果只有一个标记，自动补全为7天
  if (dates.length === 1) {
    const start = dates[0]
    const end = new Date(start)
    end.setDate(start.getDate() + 6)  // +6天 = 共7天
    return { start, end }
  }
...
export function checkIsPeriod(periodDays, dateObj) {
  ...
  const checkDate = new Date(dateObj.y, dateObj.m - 1, dateObj.d)
  return checkDate >= range.start && checkDate <= range.end
}

// 检查是否是经期最后一天（用于提醒）
export function isLastPeriodDay(periodDays, dateObj) {
  const range = getPeriodRange(periodDays)
  if (!range) return false
  
  const checkDate = new Date(dateObj.y, dateObj.m - 1, dateObj.d)
  return checkDate.getTime() === range.end.getTime()
}
